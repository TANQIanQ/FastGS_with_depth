	...
	if (deg > 0)
	{
		...
		if (deg > 1)
		{
			...
			if (deg > 2)
			{
				...
				dRGBdx += (SH_C3[0] * sh[8] * 3.f * 2.f * xy +
						   SH_C3[1] * sh[9] * yz +
	...
	float *__restrict__ dL_dinvdepths)
{
	...
	float Register_dL_dinvdepth = 0.0f;
...
#pragma unroll
	for (int i = 0; i < BLOCK_SIZE + 31; ++i)
	{
		...
		accum_invdepth_rec = my_warp.shfl_up(accum_invdepth_rec, 1);
		last_invdepth = my_warp.shfl_up(last_invdepth, 1);
		last_alpha = my_warp.shfl_up(last_alpha, 1);
		...
		if (valid_splat && valid_pixel && 0 <= idx && idx < BLOCK_SIZE)
		{
			...
			if (dL_invdepths)
			{
				const float invd = 1.f / depth_val;
				...
				Register_dL_dinvdepth += dchannel_dcolor * dL_invdepth;
			}
	...
	if (valid_splat)
	{
		...
		if (dL_invdepths)
			atomicAdd(&dL_dinvdepths[gaussian_idx], Register_dL_dinvdepth);
	}
}
